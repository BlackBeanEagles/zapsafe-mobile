/// Day 267 — Korean (ko) Translation Pack
///
/// Section D (Days 261-280): starter `assets/translations/ko.json` with 20
/// core keys, bilingual review table (en ↔ ko), per-key QA checkboxes,
/// and LTR in-app preview mock.
///
/// Tag: 🟢 FRONTEND-ONLY · loads ko.json from assets.
///
/// Route: [AppRoutes.koreanPack] → `/korean-pack`
library;
...
// ── Constants ─────────────────────────────────────────────────────────────────
const _kAccent = Color(0xFF0047A0);
const _kTabs = ['Pack', 'Review', 'Info'];
const _kJsonEncoder = JsonEncoder.withIndent('  ');
const _kAssetPath = 'assets/translations/ko.json';
...
// ── Providers ─────────────────────────────────────────────────────────────────
final _d267TabProvider = StateProvider<int>((ref) => 0);
final _d267KoJsonProvider = FutureProvider<Map<String, dynamic>>((ref) async {
  final raw = await rootBundle.loadString(_kAssetPath);
  return jsonDecode(raw) as Map<String, dynamic>;
});
final _d267ReviewedProvider = StateProvider<Set<String>>((ref) => {});
final _d267ApprovedProvider = StateProvider<bool>((ref) => false);

// ── Screen ────────────────────────────────────────────────────────────────────
class Day267KoreanPackScreen extends ConsumerWidget {
  const Day267KoreanPackScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final tab = ref.watch(_d267TabProvider);
    final reviewed = ref.watch(_d267ReviewedProvider);
    final approved = ref.watch(_d267ApprovedProvider);
    final koAsync = ref.watch(_d267KoJsonProvider);

    return Scaffold(
      backgroundColor: ZapColors.bgPrimary,
      appBar: AppBar(
        title: const Text('Day 267 · Korean Pack'),
        actions: [
          Padding(
      ...
      body: Column(
        children: [
          _TabBar(
            tab: tab,
            onSelect: (i) => ref.read(_d267TabProvider.notifier).state = i,
          ),
          Expanded(
            child: koAsync.when(
    ...
    return ListView(
      ...
      children: [
        Container(
          ...
          child: const Text(
            '🟢 FRONTEND-ONLY · Section D Day 7/20 · starter ko.json · LTR locale',
        ...
        OutlinedButton.icon(
          onPressed: () {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('ko.json copied to clipboard.')),
            );
          },
          icon: const Icon(Icons.copy_rounded, size: 16),
          label: const Text('Copy ko.json'),
        ),
      ],
    );
  }
      ...
      children: [
        ...
        FilledButton.icon(
          onPressed: !allDone || approved
              ? null
              : () {
                  ref.read(_d267ApprovedProvider.notifier).state = true;
                  HapticFeedback.mediumImpact();
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                        'Korean pack approved (mock) · ready for EasyLocalization',
                      ),
                    ),
                  );
                },
...
class _InfoTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final payload = {
      'endpoint': 'GET /api/v1/i18n/locale-pack/ko/',
    ...
    return ListView(
      ...
      children: [
        const _PolicyRow(
          ...
          title: 'Korea expansion pack',
          subtitle:
              'Starter ko.json mirrors Day 266 ja structure · 20 core keys '
        ...
        const _PolicyRow(
          ...
          subtitle:
              'Bilingual en/ko table with per-key QA checkboxes · mock approve '
        ...
        OutlinedButton.icon(
          onPressed: () {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('ko pack spec copied.')),
            );
          },
          icon: const Icon(Icons.copy_rounded, size: 16),
