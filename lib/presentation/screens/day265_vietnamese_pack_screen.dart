/// Day 265 — Vietnamese (vi) Translation Pack
///
/// Section D (Days 261-280): starter `assets/translations/vi.json` with 20
/// core keys, bilingual review table (en ↔ vi), per-key QA checkboxes,
/// and LTR in-app preview mock.
///
/// Tag: 🟢 FRONTEND-ONLY · loads vi.json from assets.
///
/// Route: [AppRoutes.vietnamesePack] → `/vietnamese-pack`
library;
...
// ── Constants ─────────────────────────────────────────────────────────────────
const _kAccent = Color(0xFFCA8A04);
const _kTabs = ['Pack', 'Review', 'Info'];
const _kJsonEncoder = JsonEncoder.withIndent('  ');
const _kAssetPath = 'assets/translations/vi.json';
...
// ── Providers ─────────────────────────────────────────────────────────────────
final _d265TabProvider = StateProvider<int>((ref) => 0);
final _d265ViJsonProvider = FutureProvider<Map<String, dynamic>>((ref) async {
  final raw = await rootBundle.loadString(_kAssetPath);
  return jsonDecode(raw) as Map<String, dynamic>;
});
final _d265ReviewedProvider = StateProvider<Set<String>>((ref) => {});
final _d265ApprovedProvider = StateProvider<bool>((ref) => false);

// ── Screen ────────────────────────────────────────────────────────────────────
class Day265VietnamesePackScreen extends ConsumerWidget {
  const Day265VietnamesePackScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final tab = ref.watch(_d265TabProvider);
    final reviewed = ref.watch(_d265ReviewedProvider);
    final approved = ref.watch(_d265ApprovedProvider);
    final viAsync = ref.watch(_d265ViJsonProvider);

    return Scaffold(
      backgroundColor: ZapColors.bgPrimary,
      appBar: AppBar(
        title: const Text('Day 265 · Vietnamese Pack'),
        actions: [
          Padding(
      ...
      body: Column(
        children: [
          _TabBar(
            tab: tab,
            onSelect: (i) => ref.read(_d265TabProvider.notifier).state = i,
          ),
          Expanded(
            child: viAsync.when(
    ...
    return ListView(
      ...
      children: [
        Container(
          ...
          child: const Text(
            '🟢 FRONTEND-ONLY · Section D Day 5/20 · starter vi.json · LTR locale',
            ...
            const Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Tiếng Việt · assets/translations/vi.json',
                    style: TextStyle(
        ...
        OutlinedButton.icon(
          onPressed: () {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('vi.json copied to clipboard.')),
            );
          },
          icon: const Icon(Icons.copy_rounded, size: 16),
          label: const Text('Copy vi.json'),
        ),
      ],
    );
  }
      ...
      children: [
                    ...
                    Checkbox(
                      ...
                      onChanged: approved
                          ...
                          : (v) {
                              ...
                              ref
                                  .read(_d265ReviewedProvider.notifier)
                                  .state = next;
                              ref.read(_d265ApprovedProvider.notifier).state =
                                  false;
                            },
                    ),
                    Expanded(
        ...
        FilledButton.icon(
          onPressed: !allDone || approved
              ? null
              : () {
                  ref.read(_d265ApprovedProvider.notifier).state = true;
                  HapticFeedback.mediumImpact();
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                        'Vietnamese pack approved (mock) · ready for EasyLocalization',
                      ),
                    ),
                  );
                },
        ...
        if (!allDone)
          Padding(
            ...
            child: TextButton(
              onPressed: () {
                ref.read(_d265ReviewedProvider.notifier).state = {
...
class _InfoTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final payload = {
      'endpoint': 'GET /api/v1/i18n/locale-pack/vi/',
      ...
      'registration': {
        'easy_localization': "Locale('vi')",
        'lang_info':
            "LangInfo(code: 'vi', nativeName: 'Tiếng Việt', flag: '🇻🇳')",
        'workflow': 'Day 262 template → Day 265 vi pack → expand to 236 keys',
      },
      'qa_checks': [
        'placeholder_parity',
    ...
    return ListView(
      ...
      children: [
        const _PolicyRow(
          ...
          title: 'Vietnam expansion pack',
          subtitle:
              'Starter vi.json mirrors Day 264 id structure · 20 core keys '
        ...
        const _PolicyRow(
          ...
          subtitle:
              'Bilingual en/vi table with per-key QA checkboxes · mock approve '
        ...
        OutlinedButton.icon(
          onPressed: () {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('vi pack spec copied.')),
            );
          },
          icon: const Icon(Icons.copy_rounded, size: 16),
