/// Day 264 — Indonesian (id) Translation Pack
///
/// Section D (Days 261-280): starter `assets/translations/id.json` with 20
/// core keys, bilingual review table (en ↔ id), per-key QA checkboxes,
/// and LTR in-app preview mock.
///
/// Tag: 🟢 FRONTEND-ONLY · loads id.json from assets.
///
/// Route: [AppRoutes.indonesianPack] → `/indonesian-pack`
library;
...
const _kAssetPath = 'assets/translations/id.json';
...
// ── Providers ─────────────────────────────────────────────────────────────────
final _d264TabProvider = StateProvider<int>((ref) => 0);
final _d264IdJsonProvider = FutureProvider<Map<String, dynamic>>((ref) async {
  final raw = await rootBundle.loadString(_kAssetPath);
  return jsonDecode(raw) as Map<String, dynamic>;
});
final _d264ReviewedProvider = StateProvider<Set<String>>((ref) => {});
final _d264ApprovedProvider = StateProvider<bool>((ref) => false);

// ── Screen ────────────────────────────────────────────────────────────────────
class Day264IndonesianPackScreen extends ConsumerWidget {
  const Day264IndonesianPackScreen({super.key});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final tab = ref.watch(_d264TabProvider);
    final reviewed = ref.watch(_d264ReviewedProvider);
    final approved = ref.watch(_d264ApprovedProvider);
    final idAsync = ref.watch(_d264IdJsonProvider);

    return Scaffold(
      backgroundColor: ZapColors.bgPrimary,
      appBar: AppBar(
        title: const Text('Day 264 · Indonesian Pack'),
        actions: [
          Padding(
      ...
      body: Column(
        children: [
          _TabBar(
            tab: tab,
            onSelect: (i) => ref.read(_d264TabProvider.notifier).state = i,
          ),
          Expanded(
            child: idAsync.when(
    ...
    return ListView(
      ...
      children: [
        Container(
          ...
          child: const Text(
            '🟢 FRONTEND-ONLY · Section D Day 4/20 · starter id.json · LTR locale',
            ...
            const Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Bahasa Indonesia · assets/translations/id.json',
                    style: TextStyle(
        ...
        OutlinedButton.icon(
          onPressed: () {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('id.json copied to clipboard.')),
            );
          },
          icon: const Icon(Icons.copy_rounded, size: 16),
          label: const Text('Copy id.json'),
        ),
      ],
    );
  }
      ...
      children: [
                    ...
                    Checkbox(
                      ...
                      onChanged: approved
                          ...
                          : (v) {
                              ...
                              ref
                                  .read(_d264ReviewedProvider.notifier)
                                  .state = next;
                              ref.read(_d264ApprovedProvider.notifier).state =
                                  false;
                            },
                    ),
                    Expanded(
        ...
        FilledButton.icon(
          onPressed: !allDone || approved
              ? null
              : () {
                  ref.read(_d264ApprovedProvider.notifier).state = true;
                  HapticFeedback.mediumImpact();
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text(
                        'Indonesian pack approved (mock) · ready for EasyLocalization',
                      ),
                    ),
                  );
                },
        ...
        if (!allDone)
          Padding(
            ...
            child: TextButton(
              onPressed: () {
                ref.read(_d264ReviewedProvider.notifier).state = {
...
class _InfoTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final payload = {
      'endpoint': 'GET /api/v1/i18n/locale-pack/id/',
      ...
      'registration': {
        'easy_localization': "Locale('id')",
        'lang_info':
            "LangInfo(code: 'id', nativeName: 'Bahasa Indonesia', flag: '🇮🇩')",
        'workflow': 'Day 262 template → Day 264 id pack → expand to 236 keys',
      },
      'qa_checks': [
        'placeholder_parity',
    ...
    return ListView(
      ...
      children: [
        const _PolicyRow(
          ...
          title: 'Indonesia expansion pack',
          subtitle:
              'Starter id.json mirrors Day 263 fa structure · 20 core keys '
        ...
        const _PolicyRow(
          ...
          subtitle:
              'Bilingual en/id table with per-key QA checkboxes · mock approve '
        ...
        OutlinedButton.icon(
          onPressed: () {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('id pack spec copied.')),
            );
          },
          icon: const Icon(Icons.copy_rounded, size: 16),
