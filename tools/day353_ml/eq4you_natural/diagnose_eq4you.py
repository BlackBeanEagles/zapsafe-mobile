"""Day 353 - is EQ4You usable, or is it ESD again with a different accent?
...
So EQ4You gets the same three tests before a single model is trained on it.

TEST 1 - DIRECTION AGREEMENT
    Per-feature signed AUC on EQ4You vs on MELD, correlated. Negative or
...
TEST 2 - CORPUS SEPARABILITY
    Can a classifier tell EQ4You rows from MELD rows? EQ4You is 8 kHz
    ...
    A; else rule B" and the EQ4You rows then contribute nothing to MELD
...
TEST 3 is the paired A/B in ab_eq4you.py, and it is the one with authority.
...
def run(tag, eq4you_path, meld_path, ykey):
    Xl, yl = load(eq4you_path, ykey)
    Xm, ym = load(meld_path)
    print("\n" + "=" * 72)
    print("=== %s  (EQ4You label set: %s) ===" % (tag, ykey))
    print("  EQ4You %s pos=%d (%.1f%%)" % (Xl.shape, yl.sum(),
                                         yl.mean() * 100))
    ...
    print("  linear EQ4You -> MELD : %.4f" % l2m)
    print("  linear MELD -> EQ4You : %.4f" % m2l)
    ...
    return {"dir_corr": round(r, 4), "dir_corr_ci95": [round(lo, 4),
            ...
            "linear_eq4you_to_meld": round(l2m, 4),
            "linear_meld_to_eq4you": round(m2l, 4), "verdict": verdict}
...
def main():
    ...
    for ykey in ("y",):
        rep["h_aggressive_2048_" + ykey] = run(
            ...
            r"D:\zapsafe\eq4you\eq4you_yin2048.npz",
            os.path.join(WORK, "h_aggressive_v4", "feat_meld_train.npz"),
            ykey)
        rep["m4_512_" + ykey] = run(
            "m4 space (512/256)",
            r"D:\zapsafe\eq4you\eq4you_yin512.npz",
