"""Day 353 - is NaturalVoices usable, or is it ESD again with a different accent?
...
So NaturalVoices gets the same three tests before a single model is trained on it.

TEST 1 - DIRECTION AGREEMENT
    Per-feature signed AUC on NaturalVoices vs on MELD, correlated. Negative or
...
TEST 2 - CORPUS SEPARABILITY
    Can a classifier tell NaturalVoices rows from MELD rows? NaturalVoices is 8 kHz
    ...
    A; else rule B" and the NaturalVoices rows then contribute nothing to MELD
...
TEST 3 is the paired A/B in ab_nv.py, and it is the one with authority.
...
def run(tag, nv_path, meld_path, ykey):
    Xl, yl = load(nv_path, ykey)
    Xm, ym = load(meld_path)
    print("\n" + "=" * 72)
    print("=== %s  (NaturalVoices label set: %s) ===" % (tag, ykey))
    print("  NaturalVoices %s pos=%d (%.1f%%)" % (Xl.shape, yl.sum(),
                                         yl.mean() * 100))
    ...
    print("  linear NaturalVoices -> MELD : %.4f" % l2m)
    print("  linear MELD -> NaturalVoices : %.4f" % m2l)
    ...
    return {"dir_corr": round(r, 4), "dir_corr_ci95": [round(lo, 4),
            ...
            "linear_nv_to_meld": round(l2m, 4),
            "linear_meld_to_nv": round(m2l, 4), "verdict": verdict}
