"""Day 353 - does adding NaturalVoices make m4 or h_aggressive better on MELD?

THE DESIGN
==========
NaturalVoices rows are TRAINING-SIDE ONLY. Both arms are scored on exactly the same
...
So NaturalVoices cannot leak into the measurement, and the only thing that differs
...
==============================
Adopt NaturalVoices only if the paired delta on the MELD eval set has a 95% CI
...
WHAT NaturalVoices CANNOT FIX, STATED UP FRONT
=====================================
h_aggressive's positive class is angry OR fearful OR disgusted. NaturalVoices has
anger only. So even in the best case NaturalVoices reinforces one third of the
...
def run(tag, base_sets, eval_path, nv_path, ykey, units):
    from sklearn.metrics import roc_auc_score
    ...
    Xl, yl, _ = load(nv_path, ykey)

    print("\n" + "=" * 72)
    print("=== %s   NaturalVoices labels: %s ===" % (tag, ykey))
    print("  base  %s pos=%d from %d corpora"
          % (Xb.shape, yb.sum(), len(base_sets)))
    print("  NaturalVoices  %s pos=%d" % (Xl.shape, yl.sum()))
    ...
    for arm in ("WITHOUT", "WITH"):
        if arm == "WITH":
            ...
            ctr = np.concatenate([cb, np.full(len(yl), "nv")])
        else:
    ...
    print("\n  PAIRED delta (WITH - WITHOUT) %+.4f 95%% CI [%+.4f, %+.4f]"
          % (mean, lo, hi))
    adopt = lo > 0.0
    print("  -> %s" % ("ADOPT NaturalVoices" if adopt else
