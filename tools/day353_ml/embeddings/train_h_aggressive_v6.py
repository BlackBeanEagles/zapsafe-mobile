...
def main():
    ...
    for f in sorted(glob.glob(os.path.join(HA, "feat_*.npz"))):
        ...
        # Day 360: TESS (CC BY-NC 4.0) and RAVDESS (CC BY-NC-SA 4.0) are
        # h_aggressive's ONLY NC corpora. Dropping them measured +0.0097 on
        # MELD eval and -0.0003 / +0.0037 on the two cross-corpus sets, so
        # the licence flag clears at no cost -- the acted corpora were very
        # slightly harmful, matching every other acted-data finding.
        if t == "meld_eval" or t.lower() in ("tess", "ravdess"):
            continue
        X, y = load(f)
        Xs.append(X)
        ys.append(y)
    ...
    # Day 360: the >=0.02 bar was authored for the v4->v5 ACCURACY question
    # ("is this recipe change worth shipping?"). v6 answers a different one:
    # "can the two NC corpora be dropped without cost?" For a licence fix the
    # bar is NON-REGRESSION, not a gain -- the accuracy change is incidental.
    # Stating the swap explicitly rather than quietly relaxing a threshold.
    ship = bool(a.mean() >= V4_GATE and a.min() >= V4_GATE - 0.005)
    print("  SHIP (licence rule: mean >= %.3f and worst seed within 0.005) "
          "-> %s" % (V4_GATE, ship))
    ...
    if ship:
        ...
        blob = conv.convert()
        open(os.path.join(HERE, "h_aggressive_v6_38.tflite"), "wb").write(blob)
        json.dump({"mean": mu.tolist(), "std": sd.tolist()},
                  open(os.path.join(HERE, "h_aggressive_v6_38_norm.json"),
                       "w"))
        rep["float16_kb"] = round(len(blob) / 1024, 1)
        print("  wrote h_aggressive_v6_38.tflite (%s KB) + norm"
              % rep["float16_kb"])
