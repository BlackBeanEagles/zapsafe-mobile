"""Day 353 - does adding MOSEI make m4 or h_aggressive better on MELD?

THE DESIGN
==========
MOSEI rows are TRAINING-SIDE ONLY. Both arms are scored on exactly the same
...
So MOSEI cannot leak into the measurement, and the only thing that differs
...
==============================
Adopt MOSEI only if the paired delta on the MELD eval set has a 95% CI
...
WHAT MOSEI CANNOT FIX, STATED UP FRONT
=====================================
h_aggressive's positive class is angry OR fearful OR disgusted. MOSEI has
anger only. So even in the best case MOSEI reinforces one third of the
...
def run(tag, base_sets, eval_path, mosei_path, ykey, units):
    from sklearn.metrics import roc_auc_score
    ...
    Xl, yl, _ = load(mosei_path, ykey)

    print("\n" + "=" * 72)
    print("=== %s   MOSEI labels: %s ===" % (tag, ykey))
    print("  base  %s pos=%d from %d corpora"
          % (Xb.shape, yb.sum(), len(base_sets)))
    print("  MOSEI  %s pos=%d" % (Xl.shape, yl.sum()))
    ...
    for arm in ("WITHOUT", "WITH"):
        if arm == "WITH":
            ...
            ctr = np.concatenate([cb, np.full(len(yl), "mosei")])
        else:
    ...
    print("\n  PAIRED delta (WITH - WITHOUT) %+.4f 95%% CI [%+.4f, %+.4f]"
          % (mean, lo, hi))
    adopt = lo > 0.0
    print("  -> %s" % ("ADOPT MOSEI" if adopt else
...
def main():
    ...
    for ykey in ("y",):
        rep["h_aggressive_" + ykey] = run(
            ...
            os.path.join(HERE, "mosei_yin2048.npz"), ykey, (64, 32))
        rep["m4_" + ykey] = run(
            "m4 vocal stress (512/256)", base_m4,
            os.path.join(WORK, "m4_m5_crosscorpus", "feat_meld_yin.npz"),
            os.path.join(HERE, "mosei_yin512.npz"), ykey, (128, 64))
