"""Day 353 - is MOSEIv2 usable, or is it ESD again with a different accent?
...
So MOSEIv2 gets the same three tests before a single model is trained on it.

TEST 1 - DIRECTION AGREEMENT
    Per-feature signed AUC on MOSEI vs on MELD, correlated. Negative or
...
TEST 2 - CORPUS SEPARABILITY
    Can a classifier tell MOSEI rows from MELD rows? MOSEI is 8 kHz
    ...
    A; else rule B" and the MOSEI rows then contribute nothing to MELD
...
TEST 3 is the paired A/B in ab_mosei.py, and it is the one with authority.
...
def run(tag, mosei_path, meld_path, ykey):
    Xl, yl = load(mosei_path, ykey)
    Xm, ym = load(meld_path)
    print("\n" + "=" * 72)
    print("=== %s  (MOSEI label set: %s) ===" % (tag, ykey))
    print("  MOSEI %s pos=%d (%.1f%%)" % (Xl.shape, yl.sum(),
                                         yl.mean() * 100))
    ...
    print("  linear MOSEI -> MELD : %.4f" % l2m)
    print("  linear MELD -> MOSEI : %.4f" % m2l)
    ...
    return {"dir_corr": round(r, 4), "dir_corr_ci95": [round(lo, 4),
            ...
            "linear_mosei_to_meld": round(l2m, 4),
            "linear_meld_to_mosei": round(m2l, 4), "verdict": verdict}
